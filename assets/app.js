(() => {
  ...
  if (typeSelect) {
    ...
    const placeholder = document.createElement("option"); placeholder.value = ""; placeholder.textContent = copy.typePlaceholder; typeSelect.append(placeholder);
    copy.productionTypes.forEach(([groupLabel, options]) => { const group = document.createElement("optgroup"); group.label = groupLabel; options.forEach(([value,label]) => { const option = document.createElement("option"); option.value = value; option.textContent = label; group.append(option); }); typeSelect.append(group); });
    const other = document.createElement("input"); other.name = "type_other"; other.placeholder = copy.typeOther; other.hidden = true; other.setAttribute("aria-label", copy.typeOther); typeSelect.after(other);
    const updateOther = () => { const visible = typeSelect.value === "other"; other.hidden = !visible; other.required = visible; if (!visible) other.value = ""; };
    typeSelect.addEventListener("change", updateOther); updateOther();
  }
