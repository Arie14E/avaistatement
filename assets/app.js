(() => {
  ...
  function saveDraft() {
    ...
    const draft={format:localCopy.draftFormat,version:"0.7",identifier:{scheme:"origin-report",value:reportId,status:"local-unregistered"},createdAt:reportEntry.createdAt,updatedAt:now,language,values};
  ...
  function validateForExport() { const requiredSteps=[1,2,3,5]; for (const stepNumber of requiredSteps) { showStep(stepNumber); if (!validateCurrent()) return false; } showStep(5); saveDraft(); return true; }
  document.querySelector("#download-json")?.addEventListener("click",()=>{ if (!validateForExport()) return; const report=payload(),safe=(report.title || "origin-report").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,""); downloadObject(report,`${safe || "origin-report"}-${reportId.toLowerCase()}.origin.json`); });
  c2paButton.addEventListener("click",()=>{ if (!validateForExport()) return; const report=payload(),current=counts(),safe=(report.title || "origin-report").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,""); const handoff={format:"origin-report-c2pa-handoff",version:"0.1",status:"unsigned-preparation",warning:c2paCopy.warning,originReport:{identifier:report.identifier,formatVersion:report.version,title:report.title,producer:report.producer,signed:report.signed.attested},summary:{includedStages:includedStages().length,notUsed:current.none,assisted:current.assisted,aiMaterialInFinalResult:current.generated,notApplicable:current.na},candidateDigitalSourceTypes:report.interoperability.iptc.candidateDigitalSourceTypes,suggestedCustomAssertion:{label:"org.originreport.declaration",kind:"Json",data:{originReportId:reportId,reportFormatVersion:report.version,summary:{assistedStages:current.assisted,generatedStages:current.generated}}},requiresAssetSpecificReview:true,reviewInstruction:c2paCopy.review,missingForContentCredential:["target media asset","asset-specific c2pa.created or c2pa.opened action","private signing key","appropriate signing certificate","cryptographic signature","optional trusted timestamp"]}; downloadObject(handoff,`${safe || "origin-report"}-${reportId.toLowerCase()}.${c2paCopy.filename}.json`); });
